...
export function generateSignaturePNG(
  ...
  for (let i = 0; i < points.length; i++) {
    ...
    const shouldBreakStroke = isLastPoint || (nextPoint && (nextPoint.timestamp - point.timestamp) > 100);
 ...
 */
export function downloadSignatureAsPNG(signature: Signature, filename?: string): void {
 ...
 */
export function isValidSignature(signature: Signature, minPoints: number = 10): boolean {
 ...
 */
export function formatSignatureDate(signature: Signature, locale: string = "ru-RU"): string {
 ...
 */
export function formatSignatureDateTime(signature: Signature, locale: string = "ru-RU"): string {
 ...
 */
export function getShortSignatureId(signature: Signature, length: number = 8): string {
...
export async function toggleUserForForgery(signature: SignatureGenuine): Promise<void> {
  try {
    ...
    if (!res.ok) {
      const msg = await res.json().catch(() => ({}));
      toast({ description: msg.error || "Ошибка изменения видимости подписи", type: "background" });
      return;
    }
    ...
    window.dispatchEvent(
      new CustomEvent("signatureUpdated", { detail: { id: signature.id, user_for_forgery } }),
    );
  } catch (error) {
...
export async function toggleModForForgery(signature: SignatureGenuine): Promise<void> {
  try {
    const res = await fetch(`/api/signatures/${signature.id}`, {
      method: "PATCH",
    ...
    window.dispatchEvent(
      new CustomEvent("signatureUpdated", { detail: { id: signature.id, mod_for_forgery } }),
    );
  }
  catch (error) {
...
export async function toggleModForDataset(signature: Signature): Promise<void> {
  ...
  try {
    ...
    window.dispatchEvent(
      new CustomEvent("signatureUpdated", { detail: { id: signature.id, mod_for_dataset } }),
    );
  }
  catch (error) {
...
export async function deleteSignature(signature: Signature): Promise<boolean> {
  // TODO: Разбить на Genuine и Forged
  const ok = await confirm({ description: "Вы уверены, что хотите удалить эту подпись?", confirmText: "Удалить", cancelText: "Отмена" });
  if (!ok) return false;
