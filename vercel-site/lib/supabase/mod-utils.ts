...
export async function searchUsersAndPseudousers(
    ...
    const [profilesPrefix, pseudousersPrefix, profilesSubstr, pseudousersSubstr] = await Promise.all([
    ...
    const listA: User[] = (profilesPrefix || []).map((p: Profile) => createProfileUser(p)) || [];
    const listB: User[] = (pseudousersPrefix || []).map((p: Pseudouser) => createPseudouserUser(p)) || [];
    const listC: User[] = (profilesSubstr || []).map((p: Profile) => createProfileUser(p)) || [];
    const listD: User[] = (pseudousersSubstr || []).map((p: Pseudouser) => createPseudouserUser(p)) || [];
...
export async function getUserGenuineSignatures(userId: string, userType: "user" | "pseudouser" = "user"): Promise<SignatureGenuine[]> {
    const client = createBrowserClient();
    const signatures = await getUserGenuineSignaturesQuery(userId, client, userType);
    return signatures;
}

export async function ensurePseudouser(name: string, source: string): Promise<{ pseudouser: Pseudouser, created: boolean }> {
    const client = createBrowserClient();
