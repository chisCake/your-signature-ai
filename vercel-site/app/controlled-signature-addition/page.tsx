...
export default function ControlledSignatureAddition() {
  ...
  const handleCreate = async () => {
    ...
    try {
      const { pseudouser, created } = await ensurePseudouser(
        latestQuery.trim(),
        'manual'
      );
      ...
      setMessage(
        created
          ? 'Пользователь создан'
          : 'Пользователь уже существовал. Выбран.'
      );
    } catch (e) {
      console.error(e);
    ...
    <>
      <div className='flex flex-col items-center justify-center h-full gap-4 p-4 px-6'>
        ...
        <div className='flex flex-col gap-4 items-center w-full'>
          <Label htmlFor='user-search' className='self-start'>
          ...
          <Button
            ...
            disabled={
              !latestQuery.trim() || exactPseudouserExists || inputChanged
            }
          >
            Создать
          </Button>
          ...
          {selectedUser && (
            <div className='w-full rounded-md border p-4 text-sm space-y-2'>
              <div className='flex items-center gap-2'>
                ...
                <Badge
                  variant={
                    selectedUser.type === 'user' ? 'default' : 'secondary'
                  }
                >
      ...
      {modalOpen && (
        <div className='fixed top-0 bottom-0 left-0 right-0 bg-black/50 z-50 w-full h-screen flex items-center justify-center'>
          <div className='flex flex-col gap-4 border border-secondary rounded-lg p-4 bg-primary-foreground m-4 max-h-[95vh] overflow-y-auto'>
            <CreateSignatureSection
              ...
              saveSignature={async opts => {
                ...
                return saveForAnotherSignature({
                  ...opts,
                  targetTable:
                    selectedUser.type === 'user' ? 'profiles' : 'pseudousers',
            ...
            <div>
              {selectedUser ? getUserName(selectedUser) : ''} - сохраненно
              подписей: {signatures.length}
            </div>
          </div>
        </div>
      )}
