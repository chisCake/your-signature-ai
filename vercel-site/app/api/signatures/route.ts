...
export async function POST(req: NextRequest) {
    ...
    const { csvData, inputType, userForForgery = false, targetTable, targetId } = parse.data;
...
export async function PATCH(req: NextRequest) {
    ...
    if (json?.userId !== undefined) {
        ...
        if (targetUserId !== user?.sub) {
            ...
            if (targetUserRole === "mod" || targetUserRole === "admin") {
                return NextResponse.json({ error: "Insufficient permissions for target user" }, { status: 403 });
            }
        }
    }
    ...
    if (json?.userForForgery === undefined && json?.modForForgery === undefined && json?.modForDataset === undefined) {
