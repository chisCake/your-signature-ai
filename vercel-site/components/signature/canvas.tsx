...
const Canvas = forwardRef<CanvasRef, CanvasProps>(
  ({ className = '', canvasClassName = '', showInputTypeIcon = true }, ref) => {
    ...
    const detectInputType = useCallback(
      (e: React.MouseEvent | React.TouchEvent | PointerEvent): InputType => {
    ...
    const isInputTypeCompatible = useCallback(
      (newInputType: InputType): boolean => {
    ...
    const startDrawing = useCallback(
      (e: React.PointerEvent<HTMLCanvasElement>) => {
        ...
        const timestamp =
          signatureDataRef.current.length === 0
            ? 0
