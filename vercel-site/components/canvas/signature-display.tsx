...
const SignatureDisplay = memo(
  React.forwardRef<SignatureDisplayRef, SignatureDisplayProps>(
    (
      {
        signatureData,
        className = '',
      ...
      const getCenteredPoints = useCallback(
        (
          data: SignaturePoint[][],
          canvasWidth: number,
          canvasHeight: number
          ...
          const bufferX =
            bufferSize !== undefined ? bufferSize : defaultBufferX;
          const bufferY =
            bufferSize !== undefined ? bufferSize : defaultBufferY;
      ...
      const drawSignature = useCallback(
        (
          ...
          centeredData.forEach((signature, signatureIndex) => {
            ...
            const pointsToDraw =
              upToIndex !== undefined
                ? signature.slice(0, upToIndex + 1)
                : signature;
            const color =
              normalizedColors[signatureIndex % normalizedColors.length] ||
              SIGNATURE_COLORS[signatureIndex % SIGNATURE_COLORS.length];
            const width =
              normalizedWidths[signatureIndex % normalizedWidths.length] || 2;
              ...
              for (let i = 1; i < pointsToDraw.length; i++) {
                ...
                if (timeDiff <= 100) {
                  ...
                  if (i < pointsToDraw.length - 1) {
                    ...
                    const cp1x =
                      prevPoint.x + (currentPoint.x - prevPoint.x) * 0.5;
                    const cp1y =
                      prevPoint.y + (currentPoint.y - prevPoint.y) * 0.5;
      ...
      const animateSignature = useCallback(() => {
        const totalPoints = normalizedData.reduce(
          (sum, sig) => sum + sig.length,
          0
        );
      ...
      return (
        <div
          ref={containerRef}
          className={`relative inline-block border-2 border-gray-200 rounded-lg bg-white shadow-lg ${className}`}
        >
          {isLoading ? (
            <div
              className={`flex items-center justify-center bg-gray-50 rounded-md ${canvasClassName || DEFAULT_CANVAS_SIZE}`}
            >
